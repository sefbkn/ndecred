    ...
    public class Command
    {
	    ...
	    public static Command Decode(byte[] bytes)
	    {
		    if(bytes.Length != CommandSizeBytes)
			    throw new Exception($"Expected command bytes to be exactly equal to {CommandSizeBytes} bytes.  Actual size {bytes.Length}");
	    ...
	    public byte[] Encode()
	    {
		    ...
		    if (Name.Length > bytes.Length)
			    throw new Exception($"Command message name is larger that the maximum allowed size {CommandSizeBytes} bytes");
